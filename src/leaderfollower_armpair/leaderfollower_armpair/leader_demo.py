...
class FollowerArm(Node):
    ...
    def query_servo_current_angle(self):
        ...
        if self.leader_Servo.uservo.button_is_on != self.last_button:
            ...
            if self.leader_Servo.uservo.button_is_on:
                if self.leader_Servo.uservo.lock_state :
                    self.leader_Servo.uservo.lock_state = False
                else:
                    self.leader_Servo.uservo.lock_state = True
    ...
    def publish_current_angle(self):
        ...
        for i in range(7):
            self.current_angle[i] = self.leader_Servo.uservo.servos[i].angle_monitor
            JointState_msg.name.append(self.leader_Servo.JOINT_[i])
            JointState_msg.position.append(self.leader_Servo.robo770_servoangle2jointstate(servo_id=i, servo_angle=self.current_angle[i]))
